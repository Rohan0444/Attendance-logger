...
export async function onboardStudent(req, res) {
    ...
    const {ImgURL, department, semester} = req.body;

    try{
        if( !ImgURL || !department || !semester) {
        ...
        const faceEncoding = await getEmbedding(ImgURL);
        const updatedStudent = await Student.findByIdAndUpdate(
            studentId,
            {
                ...
                semester,
                isOnboarded: true,
                profilePhotoUrl: ImgURL, // Store paths of uploaded photos
                faceEncoding: faceEncoding, // Store the averaged encoding
            },
            { new: true }
        );
        if (!updatedStudent) {
